<x-layouts.app>
    <x-app.container class="lg:space-y-6">
        
        <x-app.heading
            :title="__('dashboard.messages_section.title')"
            :description="__('dashboard.messages_section.description')"
            :border="false"
        >
            <x-slot name="actions">
                @if($unreadCount > 0)
                    <div class="inline-flex items-center px-4 py-2 bg-orange-100 text-orange-800 font-medium rounded-lg">
                        <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"/>
                            <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z"/>
                        </svg>
                        {{ __('dashboard.messages_section.unread_count', ['count' => $unreadCount]) }}
                    </div>
                @endif
            </x-slot>
        </x-app.heading>

        @if(session('success'))
            <div class="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg">
                {{ session('success') }}
            </div>
        @endif

        @if($messages->isEmpty())
            <div class="bg-white rounded-lg shadow p-8 text-center">
                <svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                </svg>
                <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('dashboard.messages_section.no_messages') }}</h3>
                <p class="text-gray-600 mb-4">{{ __('dashboard.messages_section.no_messages_desc') }}</p>
                <a href="/property-listings" 
                   class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors">
                    {{ __('dashboard.messages_section.view_listings') }}
                </a>
            </div>
        @else
            <div class="space-y-3">
                @foreach($messages as $message)
                    <div class="bg-white rounded-lg shadow-sm border {{ $message->is_read ? 'border-gray-200' : 'border-orange-300 bg-orange-50/30' }} hover:shadow-md transition-shadow">
                        <a href="{{ route('dashboard.messages.show', $message->id) }}" class="block p-5">
                            <div class="flex items-start gap-4">
                                <!-- Icon/Badge -->
                                <div class="flex-shrink-0">
                                    @if($message->is_read)
                                        <div class="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
                                            <svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 19v-8.93a2 2 0 01.89-1.664l7-4.666a2 2 0 012.22 0l7 4.666A2 2 0 0121 10.07V19M3 19a2 2 0 002 2h14a2 2 0 002-2M3 19l6.75-4.5M21 19l-6.75-4.5M3 10l6.75 4.5M21 10l-6.75 4.5m0 0l-1.14.76a2 2 0 01-2.22 0l-1.14-.76"/>
                                            </svg>
                                        </div>
                                    @else
                                        <div class="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center">
                                            <svg class="w-6 h-6 text-orange-600" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"/>
                                                <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z"/>
                                            </svg>
                                        </div>
                                    @endif
                                </div>

                                <!-- Content -->
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-4 mb-2">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-1">
                                                <h3 class="text-base font-semibold text-gray-900">{{ $message->name }}</h3>
                                                @if(!$message->is_read)
                                                    <span class="px-2 py-0.5 text-xs font-medium bg-orange-100 text-orange-800 rounded-full">
                                                        {{ __('dashboard.messages_section.new_badge') }}
                                                    </span>
                                                @endif
                                            </div>
                                            <p class="text-sm text-gray-600">{{ $message->email }}</p>
                                        </div>
                                        <div class="text-right flex-shrink-0">
                                            <p class="text-xs text-gray-500">{{ $message->created_at->diffForHumans() }}</p>
                                            <p class="text-xs text-gray-400">{{ $message->created_at->format('d/m/Y H:i') }}</p>
                                        </div>
                                    </div>

                                    <!-- Property Info -->
                                    <div class="mb-2 p-2 bg-blue-50 rounded-md">
                                        <p class="text-sm font-medium text-blue-900">{{ $message->propertyListing->title }}</p>
                                        <p class="text-xs text-blue-600">{{ $message->propertyListing->city }}, {{ $message->propertyListing->state }}</p>
                                    </div>

                                    <!-- Message Preview -->
                                    <p class="text-sm text-gray-700 line-clamp-2">{{ $message->message }}</p>

                                    <!-- Contact Info Preview -->
                                    @if($message->phone)
                                        <div class="mt-2 flex items-center gap-2 text-xs text-gray-500">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                                            </svg>
                                            {{ $message->phone }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                        </a>

                        <!-- Quick Actions -->
                        <div class="border-t border-gray-100 px-5 py-3 bg-gray-50/50 flex items-center justify-between">
                            <div class="flex gap-2">
                                <a href="mailto:{{ $message->email }}" 
                                   class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-50 rounded-md transition-colors"
                                   onclick="event.stopPropagation();">
                                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                                    </svg>
                                    Email
                                </a>
                                @if($message->phone)
                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $message->phone) }}" 
                                       target="_blank"
                                       class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-green-600 hover:bg-green-50 rounded-md transition-colors"
                                       onclick="event.stopPropagation();">
                                        <svg class="w-4 h-4 mr-1.5" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                                        </svg>
                                        WhatsApp
                                    </a>
                                @endif
                            </div>

                            <div class="flex gap-2">
                                @php
                                    $seoService = app(\App\Services\SeoService::class);
                                    $propertySlug = $seoService->generatePropertySlug($message->propertyListing);
                                @endphp
                                <a href="{{ route_localized('property.show', ['id' => $message->propertyListing->id, 'slug' => $propertySlug]) }}" 
                                   target="_blank"
                                   class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-100 rounded-md transition-colors"
                                   onclick="event.stopPropagation();">
                                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                                    </svg>
                                    {{ __('dashboard.messages_section.view_listing') }}
                                </a>
                            </div>
                        </div>
                    </div>
                @endforeach
            </div>

            <div class="mt-6">
                {{ $messages->links() }}
            </div>
        @endif

    </x-app.container>
</x-layouts.app>
